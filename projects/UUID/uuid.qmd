...
1. **Filesystem**
    ...
    2. Unique folder name for each analysis (basepipe, triopipe or annopipe). Multiple analyses of the same type are
       possible.
...
When UUIDs are used in `.sample` and `.analysis` files, we need to consider the reanalysis scenario. For reanalyses, 
...
files and in the samples and alalyses-work folder names. The UUID will end up in Neo4j database.
...
\begin{enumerate}
    \item Re-prep or re-sequencing or re-demultiplexing will not be distinguished becuase the same UUID will be used.
\end{enumerate}
\end{tcolorbox}
