---
title: "WGS Upscaling - Data Transfer Capacity Evaluation"
...
Sequencing data and NSC pipeline results are stored at NSC and must be transferred to TSD. Due to the large data volume,
the transfer is managed by `nsc-exporter`, which checks for new data every 10 minutes and transfers it using `s3cmd put` via
TSD's `s3api` wrapper.
...
To assess the data transfer speed from NSC to TSD, we analyzed historical transfer records from the `nsc-exporter` log
between 
[`r strptime(min(data$datetime), "%Y-%m-%d %H:%M:%S")`]{style="font-family:Quicksand;"} 
and 
[`r strptime(max(data$datetime), "%Y-%m-%d %H:%M:%S")`]{style="font-family:Quicksand;"} 


::: {.callout-tip collapse="true"}

### Examples
Expand to view examples of collected file transfer records
...
library(gdata)
```
The transferred file sizes ranges from 
  [`r humanReadable(min(data$bytes))`]{style="font-family:Quicksand;"} to 
  [`r humanReadable(max(data$bytes))`]{style="font-family:Quicksand;"},
with an average of [`r humanReadable(mean(data$bytes))`]{style="font-family:Quicksand;"},
a median of [`r humanReadable(median(data$bytes))`]{style="font-family:Quicksand;"},
and a standard deviation of [`r humanReadable(sd(data$bytes))`]{style="font-family:Quicksand;"}.
...
ggplot(
  ...
  geom_histogram(fill = "#F05A7E", bins = 200) +
  labs(
    x = "file size (bytes)",
    y = "number of files",
...
ggplot(
  ...
  geom_histogram(bins = 200, fill = "#125B9A") + 
  labs(x = "bytes/s", y = "number of files", title = "Transfer Speed") + 
  scale_x_log10(labels = label_bytes()) +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))
```
:::
::::
### Per File Transfer Time
...
```{r} 
#| fig-height: 3
x_label <- "transfer time (seconds)"
ggplot(
  data,
  aes(x = seconds)
) +
  geom_histogram(bins = 200, fill = "#0B8494") +
  labs(x = x_label, y = "number of files", title = "Transfer Time") +
...
### Transfer speed and time against file size for all files {#sec-correlation-filesize-speed-all}
<hr>
...
py_df = pd.read_csv(
    "data.csv", sep=",", parse_dates=["datetime"], date_format="%Y-%m-%d %H:%M:%S.%f"
)
py_df["speed_humanize"] = py_df["speed"]
py_df["speed"] = py_df["speed"].apply(deHumanReadable)
py_df["project_type"] = py_df["project"].apply(
    lambda x: re.match(r"[a-zA-Z]+", x).group()
)
fig = px.scatter(
    py_df,
    x="bytes",
...
fig.update_layout(
    xaxis=dict(
        tickformat=".2s",  # Use SI prefixes with two significant digits
    )
)
...
ggplot(py$py_df, aes(x = bytes, y = seconds, color = project_type)) +
  ...
  labs(
    x = "file size (bytes)",
    y = "transfer time (seconds)",
    title = "transfer time vs file size"
  ) +
  theme_minimal() +
...
```
### Transfer speed and time against file size for small files only
<hr>
...
```{python} 
#| label: fig-per-day-total-transfer-sep
#| fig-cap: Daily total data transferred in September
#| cache: false
# Convert bytes to TB
def bytes_to_tb(bytes_value):
    return round(bytes_value / (1000 ** 4), 3)

py_df_sep_copy = py_df_sep.copy()
py_df_sep_copy['datetime'] = pd.to_datetime(py_df_sep_copy['datetime'])
daily_sums_sep = py_df.groupby(py_df_sep_copy['datetime'].dt.date)['bytes'].sum().reset_index()
daily_sums_sep['size_tb'] = daily_sums_sep['bytes'].apply(bytes_to_tb)
...
fig = px.bar(
    ...
    x='datetime',
    y='size_tb',
    title='Daily total data transferred (TB)',
    labels={
        'datetime': 'Day',
        'size_tb': f'Data transferred (TB)'
    }
)
...
```{python} 
#| label: fig-per-day-total-transfer-oct
#| fig-cap: Daily total data transferred in October
#| cache: false
py_df_oct_copy = py_df_oct.copy()
py_df_oct_copy['datetime'] = pd.to_datetime(py_df_oct_copy['datetime'])
daily_sums_oct = py_df.groupby(py_df_oct_copy['datetime'].dt.date)['bytes'].sum().reset_index()
daily_sums_oct['size_tb'] = daily_sums_oct['bytes'].apply(bytes_to_tb)
...
```{python} 
#| label: fig-per-day-total-transfer-nov
#| fig-cap: Daily total data transferred in November
#| cache: false

py_df_nov_copy = py_df_nov.copy()
py_df_nov_copy['datetime'] = pd.to_datetime(py_df_nov_copy['datetime'])
daily_sums_nov = py_df.groupby(py_df_nov_copy['datetime'].dt.date)['bytes'].sum().reset_index()
daily_sums_nov['size_tb'] = daily_sums_nov['bytes'].apply(bytes_to_tb)
...
px.bar(
    ...
    y=["small", "large"],
    labels={
        'boundary': 'small file definition',
...
//| echo: false
//| panel: sidebar
md`Assuming continuous transfer at speed (<code>MB/s</code>):`
...
md`<code>${textcolor(volume_hour.toString().padStart(9,'\u00A0'), 'MediumSeaGreen')}</code> data can be transferred per **hour**.`
md`<code>${textcolor(volume_day.toString().padStart(9,'\u00A0'), 'SlateBlue')}</code> data can be transferred per **day**.`
md`<code>${textcolor(volume_week.toString().padStart(9,'\u00A0'), 'Chocolate')}</code> data can be transferred per **week**.`
md`<code>${textcolor(volume_month.toString().padStart(9,'\u00A0'), 'Crimson')}</code> data can be transferred per **month**.`
```
