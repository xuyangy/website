...
def md5(x):
  if (x.time_gap - x.seconds) > 30 and (x.time_gap - x.seconds) < 600:
...
px.scatter(
    ...
    color="project",
    symbol="project_type",
    hover_data=["size", "time_gap", "file_extension"],
...
px.scatter(
    ...
    color="project",
    symbol="project_type",
    hover_data=["size", "seconds", "file_extension"],
    log_y=False,
...
r_py_df_sep %>% 
  mutate(r_datetime=as.POSIXct(datetime, format="%Y-%m-%d %H:%M:%S")) %>% 
    ...
    labs(x = "datetime", y = "md5sum check time", title = "Time used for md5sum check") +
    theme(plot.title = element_text(hjust = 0.5)) +
    guides(color = guide_legend(title = "Project Type")) + 
    scale_color_manual(values = c(
      "wgs" = "#636EFA",
...
* The nsc-exporter will sleep 10 minutes before checking for new data to transfer. In October, nsc-exporter slept [`r r_slept_times_oct`]{style="font-family:Krona One"} times, totally [`r r_ttt_oct_sleep_humanize`]{style="font-family:Quicksand;"}.
...
* The nsc-exporter will sleep 10 minutes before checking for new data to transfer. In November, nsc-exporter slept [`r r_slept_times_nov`]{style="font-family:Krona One"} times, totally [`r r_ttt_nov_sleep_humanize`]{style="font-family:Quicksand;"}.
...
[^md5sum]: Skipped wgs records where md5sum check time equals/close to zero, so project types with small files/folders are shown.
