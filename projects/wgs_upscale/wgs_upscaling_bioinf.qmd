...
ggplot(
    ...
    labs(x = "file size (bytes)", y = "count", title = "Filesize") + theme_minimal()
ggplot(
    data,
    aes(x = bytes)
) +
    geom_histogram(fill = "#fc03d3", bins = 200) +
    labs(x = "file size (bytes)", y = "count (log2)", title = "Filesize (log2)") + scale_y_continuous(trans='log2') + theme_minimal()
```
:::
::::
#### Transfer Speed
...
ggplot(
    ...
    geom_histogram(bins=200, fill="#0bd440") + 
    labs(x = "bytes/s", y = "count (log2)", title = "Transfer Speed (log2)") + scale_y_continuous(trans='log2') + theme_minimal()
```
:::
::::
#### Transfer Time
...
ggplot(
    ...
    geom_histogram(bins=200, fill="#e3571b") + 
    labs(x = "transfer time(per file)", y = "count (log2)", title = "Transfer Time (log2)") + scale_y_continuous(trans='log2') + theme_minimal()
```
:::
::::
:::
...
fig = px.scatter(py_df, x="bytes", y="speed", labels={'x': 'file size', 'y': 'transfer speed'}, opacity=0.3, color="project_type", height=500)
fig.show()
```
